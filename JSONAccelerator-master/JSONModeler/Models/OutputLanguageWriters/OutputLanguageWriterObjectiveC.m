...
- (BOOL)writeClassObjects:(NSDictionary *)classObjectsDict toURL:(NSURL *)url options:(NSDictionary *)options generatedError:(BOOL *)generatedErrorFlag {
    ...
    for (ClassBaseObject *base in files) {
        ...
        if ([base.className isEqualToString:@"InternalBaseClass"]||[base.className isEqualToString:@"iDProperty"]) {
...
- (NSString *)propertyNameForObject:(ClassPropertiesObject *)propertyObject inClass:(ClassBaseObject *)classObject fromReservedWord:(NSString *)reservedWord {
    ...
    if ([reservedWord isEqualToString:@"id"]) {
        //sld_change
        classObject.className = @"iDProperty";
...
- (NSString *)setterForProperty:(ClassPropertiesObject *)property {
    ...
    if (property.isClass && (property.type == PropertyTypeDictionary || property.type == PropertyTypeClass)) {
        ...
        NSString *templateString = [[NSString alloc] initWithContentsOfFile:interfaceTemplate encoding:NSUTF8StringEncoding error:nil];
#else 
        NSString *templateString = @"       self.{SETTERNAME} = [GlobalMethod exchangeDic:[dict objectForKey:{JSONNAME}] toAryWithModelName:@\"{REFERENCE_CLASS}\"];\n";
#endif
        ...
        if (property.type == PropertyTypeInt) {
            setterString = [setterString stringByAppendingFormat:@"[dict intValueForKey:%@];\n", [self stringConstantForProperty:property]];
        } else if (property.type == PropertyTypeDouble) {
            setterString = [setterString stringByAppendingFormat:@"[dict doubleValueForKey:%@];\n", [self stringConstantForProperty:property]];
        } else if (property.type == PropertyTypeBool) {
            setterString = [setterString stringByAppendingFormat:@"[dict boolValueForKey:%@];\n", [self stringConstantForProperty:property]];
        } else if (property.type == PropertyTypeString) {
            // It's a normal class type
            setterString = [setterString stringByAppendingFormat:@"[dict stringValueForKey:%@];\n", [self stringConstantForProperty:property]];
        }else if (property.type == PropertyTypeArray) {
            // It's a normal class type
            setterString = [setterString stringByAppendingFormat:@" [GlobalMethod exchangeDic:[dict objectForKey:%@] toAryWithModelName:<#(NSString *)#>];\n", [self stringConstantForProperty:property]];
        }else if (property.type == PropertyTypeDictionary) {
            // It's a normal class type
            setterString = [setterString stringByAppendingFormat:@"[dict dictionaryValueForKey:%@];\n", [self stringConstantForProperty:property]];
        }else{
